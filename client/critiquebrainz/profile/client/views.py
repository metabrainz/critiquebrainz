...
def edit_handler(client_id):
    form = ClientForm()
    client = api.get_client(client_id, current_user.access_token)
    if form.validate_on_submit():
        try:
            message = api.update_client(client_id, current_user.access_token,
...
def delete_handler(client_id):
    try:
        message = api.delete_client(client_id, current_user.access_token)
    except APIError as e:
        flash(e.desc, 'error')
    else:
...
def token_delete_handler(client_id):
    try:
        message = api.delete_token(client_id, current_user.access_token)
    except APIError as e:
        flash(e.desc, 'error')
    return redirect(url_for('.index'))
