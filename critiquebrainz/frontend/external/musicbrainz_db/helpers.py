...
def entity_relation_helper(db, target_type, source_type, source_entity_id):
    ...
    if relation.entity0.property.mapper.class_ == relation.entity1.property.mapper.class_:
        links = get_relationship_links(relation, query, "entity0", "entity1", source_entity_id)
        links.extend(get_relationship_links(relation, query, "entity1", "entity0", source_entity_id))
    else:
        if source_model == relation.entity0.property.mapper.class_:
            links = get_relationship_links(relation, query, "entity0", "entity1", source_entity_id)
        else:
            links = get_relationship_links(relation, query, "entity1", "entity0", source_entity_id)
    return links


def get_relationship_links(relation, query, source_attr, target_attr, source_entity_id):
