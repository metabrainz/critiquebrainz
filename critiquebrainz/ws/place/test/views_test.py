...
class PlaceViewsTestCase(WebServiceTestCase):

    def setUp(self):
        super(PlaceViewsTestCase, self).setUp()

        self.place_id1 = "4352063b-a833-421b-a420-e7fb295dece0"
        self.place_id2 = "853b36f9-8806-459c-9480-0766b8f9354b"
    ...
    def test_place_endpoint(self):
        ...
        response = self.client.get('/place/4352063b-a833-421b-a420-e7fb295dece0')
        ...
        # Test for a place that doesn't exist
        response = self.client.get('/place/4352063b-a833-421b-a420-e7fb295dece9')
        self.assert404(response)

    def test_place_user_reviews(self):
        review = self.create_dummy_review()
        response = self.client.get('/place/4352063b-a833-421b-a420-e7fb295dece0?username=%s' % self.user.musicbrainz_username)

        self.assert200(response)
        self.assertIn(review['text'], response.json['user_review']['text'])

    def test_user_cache_tracking(self):
        track_key = cache.gen_key("entity_api", self.place_id2, self.user.musicbrainz_username, "user_review")

        # Make sure the cache is empty
        self.client.get('/place/853b36f9-8806-459c-9480-0766b8f9354b?username=%s' % self.user.musicbrainz_username)
